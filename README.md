<p align="center">
  <img src="Quorum.png" width="120" alt="Quorum logo">
</p>

<h1 align="center">Quorum</h1>

<p align="center"><b>Snap your pharmacy bill. A committee of AI agents finds cheaper generics — and catches the unsafe swaps.</b></p>
...
Indians overpay for medicine every day, even though a cheaper generic with the *same* active ingredient usually exists. Apps that suggest generics already exist — but they make you type the drug name, and none of them tell you when a swap is **unsafe**.

**Quorum** reads a photographed pharmacy bill with Gemma 4 vision, matches each medicine to cheaper generic equivalents from an official price catalogue, and then runs a **committee of Gemma 4 agents** that independently verify every substitution — returning the cheaper option *with a confidence score* and flagging anything dangerous.

The tagline says it best: **an AI that tells you when *not* to trust it.**

## On one real rheumatoid-arthritis bill

- 📷 **Read 11 / 12 line items** from a photo in ~1.3s (Gemma 4 vision).
...
- ⚠️ **Flagged methotrexate** as a narrow-therapeutic-index drug needing doctor supervision.

## Why it's different — the Safety Quorum
...
- **Four method-diverse Gemma 4 agents** judge each substitution from different angles — *identity* (is the drug correctly read?), *formulation* (route / modified-release / salt form), *clinical* (narrow-therapeutic-index, prescription status), and a *skeptic* red-teaming the swap.
- **One-way ratchet:** the committee can only ever *add* caution — lower confidence, raise a flag, or say "ask a pharmacist." It can never override the database, invent a drug, or change a price. So a hallucinated caution is a safe failure.
...
**~18× faster — same model, only the silicon changes.** ([code/bench_hardware.py](code/bench_hardware.py) reproduces it; the app has an in-app engine switch to show it live.)

## Architecture

```
┌──────────────── Flutter app (iOS + Android) ───────────────┐     ┌──────── Backend (stdlib Python, HTTPS) ────────┐
│ Capture bill → on-device OCR (private, default)             │     │ • brand → salt → cheaper generic matcher       │
│         └→ or opt-in cloud scan (Gemma 4 vision)            │ ──▶ │ • Schedule H/H1/X safety classification        │
│ Results: savings + per-swap verdict chips + confidence      │ text│ • Safety Quorum (4 Gemma agents on Cerebras)   │
│ Item detail: reasoning, flags, "checked by Gemma 4"         │ ◀── │ • Jan Aushadhi prices + nearby pharmacies      │
└─────────────────────────────────────────────────────────────┘ JSON└────────────────────────────────────────────────┘
```
...
- **Privacy:** by default the photo is OCR'd **on-device** and never uploaded — only extracted text is sent. Cloud OCR (Gemma 4 vision) is an explicit opt-in.
- **API:** TLS + HMAC-signed requests (replay-resistant), strict input validation, read-only DB, no content logging.
...
Quorum is decision **support**, not medical advice. Prices are estimates from public catalogues — confirm with your pharmacist. Substituting a generic is a decision for you and your doctor; the app deliberately flags prescription-only and narrow-therapeutic-index drugs for professional review.
